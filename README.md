...
InventOps simulates a multi-warehouse inventory management problem where an LLM agent
...
| **📈 Episodes** | Reward-per-step curves · Action distribution pie · Reward components · Failure reasons |
| **🔁 RLVR Loop** | Score progression per prompt round · min/max band · Failure type breakdown |
| **⚡ Inference** | LLM latency histogram (p50/p95/p99) · Parse error rate over time · Raw step log |

The sidebar provides task filtering and optional 10-second auto-refresh.
